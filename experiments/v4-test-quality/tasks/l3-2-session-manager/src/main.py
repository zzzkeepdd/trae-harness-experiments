...
class SessionManager:
    def __init__(self, secret_key, token_ttl=3600):
        ...
        self._users = {}; self._users_by_name = {}
    ...
    def register(self, username, password):
        ...
        with self._lock:
            if username in self._users_by_name: raise ValueError(f"User '{username}' exists")
            uid = str(uuid.uuid4())
    ...
    def login(self, username, password):
        with self._lock:
            ...
            if uid is None: raise ValueError("User not found")
            user = self._users[uid]
            if not self._verify_password(password, user.password_hash): raise ValueError("Invalid password")
            if uid in self._sessions and self._sessions[uid] is not None:
                self._blacklist.add((uid, self._sessions[uid]))
            token = self._generate_token(user)
            self._sessions[uid] = token
            return token
    ...
    def verify_token(self, token):
        try:
            ...
            hb, pb, sb = parts
            header = json.loads(self._b64_decode(hb))
            ...
            tk = (payload["user_id"], token)
            with self._lock:
                if tk in self._blacklist: return None
                if payload["user_id"] not in self._sessions or self._sessions[payload["user_id"]] != token: return None
    ...
    def refresh_token(self, old_token):
        ...
        with self._lock:
            ...
            self._blacklist.add((payload.user_id, old_token))
            user = self._users[payload.user_id]
            new_token = self._generate_token(user)
            self._sessions[payload.user_id] = new_token
            return new_token

    def logout(self, user_id):
        with self._lock:
            if user_id in self._sessions:
                self._blacklist.add((user_id, self._sessions[user_id]))
